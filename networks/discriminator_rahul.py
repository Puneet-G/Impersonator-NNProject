...
class PatchDiscriminator(NetworkBase):
    ...
    def __init__(self, input_nc, ndf=64, n_layers=3, norm_type='batch', use_sigmoid=False, sn=False):
        ...
        sn_conv_l1 = SpectralNorm(conv_l1)
        print('Spectrl Norm=', sn)
        if sn:
            sequence = [sn_conv_l1, nn.LeakyReLU(0.2, True)]
        else:
        ...
        for n in range(1, n_layers):  # gradually increase the number of filters
            ...
            sn_conv_l2 = SpectralNorm(conv_l2)
            if sn:
                sequence += [
                sn_conv_l2,
        ...
        sn_conv_l3 = SpectralNorm(conv_l3)
        if sn:
            sequence += [
            sn_conv_l3,
            norm_layer(ndf * nf_mult),
        ...
        sn_conv_l4 = SpectralNorm(conv_l4)
        if sn:
            sequence += [sn_conv_l4]
        else:
